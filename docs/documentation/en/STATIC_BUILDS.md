...
The workspace pins `ort` with **`download-binaries`** so `cargo build` can fetch ONNX Runtime when needed. That is convenient for development but may be undesirable for **air-gapped** or **reproducible** forensic kit builds.
...
3. **Document the artifact** — For each release, record the ONNX Runtime version and path next to the binary (see [Forensic operation policy](../en/FORENSIC_POLICY.md) for portable layout).
