...
examined machine) configures **three tiers** independently. See **`doc/INSTALLER.md`** for full behaviour (offline vs online mode, progress, logging).
...
1. Confirm **Settings** shows hash lookups enabled and paths under your **USB / portable** `data/hashsets/`.
2. Create an empty file in a scanned folder: `touch …/nsrl-empty-test`
3. Run analysis; the results header **Hash DB** line should count at least one **known-good** hit for that file.

### Manual download (alternative to the installer option)

```bash
chmod +x scripts/download-nsrl-curated-demo.sh
./scripts/download-nsrl-curated-demo.sh /path/to/USB/data/nsrl
```

Then set the `.sqlite` path in **Settings** and apply. Source: [NIST — RDSv3 Demo Set](https://www.nist.gov/itl/ssd/software-quality-group/national-software-reference-library-nsrl/nsrl-download/current-rds).
...
Programmatically, the desktop application exposes hash-set configuration through its public analysis API (see the SDK and host documentation in the full development tree if you have access).

## Architecture note

Hash lookups run in the **host** with direct file access and optional SQLite. They are **not** implemented as WASM plugins: the sandboxed plugin model is not used for this stage.
